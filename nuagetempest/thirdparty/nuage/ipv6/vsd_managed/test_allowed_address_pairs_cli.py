...
class VSDManagedAllowedAddresPairsCLITest(BaseNuageNetworksCLITestCase, VsdTestCaseMixin):
    ...
    def test_cli_create_address_pair_l2domain_no_mac(self):
        ...
        cli_network, cli_subnet4, cli_subnet6 = self._cli_create_os_l2_vsd_unmanaged_dualstack_subnet(vsd_l2_subnet,
                                                                                                      cidr4=cidr4,
                                                                                                      cidr6=cidr6)
        ...
        #     IP@ = fixed-IP+5
        #     no MAC address
        port_fixed_ip = str(IPAddress(IPNetwork(cli_subnet4['cidr']).first +10))
        ...
        port_fixed_ip4 = str(IPAddress(IPNetwork(cli_subnet4['cidr']).first +20))
        aap_fixed_ip4 = str(IPAddress(port_fixed_ip) + 25)

        port_fixed_ip6 = str(IPAddress(IPNetwork(cli_subnet6['cidr']).first +20))
        ...
        addrpair_port_dual = self.create_port_with_args(
            ...
            "--allowed_address-pairs type=dict list=true ip_address=" + aap_fixed_ip4 + " ip_address=" + aap_fixed_ip6)
        ...
        self.cli_check_show_port_allowed_address_fields(
            ...
            addrpair_port_dual['mac_address'])
        # And no corresponding MultiVIP on the VSD
        port_ext_id6 = self.nuage_vsd_client.get_vsd_external_id(addrpair_port_dual['id'])
        ...
        self.assertIsNone(nuage_vport_dual[0]['multiNICVPortID'], "multiNICVPortID is not empty while it should be")
        ...
        self.assertEmpty(show_port['allowed_address_pairs'],
                         "Removed allowed-address-pair still present in port (%s)" % addrpair_port_dual['id'])
        pass
    ...
    def test_cli_create_address_pair_l2domain_with_mac(self):
        ...
        aap_fixed_ip4 = str(IPAddress(port_fixed_ip) + 25)
        port_fixed_ip6 = str(IPAddress(IPNetwork(vsd_l2_subnet['IPv6Address'])) + 10)
        aap_fixed_ip6 = str(IPAddress(port_fixed_ip6) + 5)
        addrpair_port_dual = self.create_port_with_args(
            cli_network['name'],
            "--name aap-port6-1",
            ...
            " ip_address=" + aap_fixed_ip4 + ",mac_address=" + VALID_MAC_ADDRESS_2A +
            " ip_address=" + aap_fixed_ip6 + ",mac_address=" + VALID_MAC_ADDRESS_2B)
        self.addCleanup(self._delete_port, addrpair_port_dual['id'])
        self.ports.remove(addrpair_port_dual)
    ...
    def test_cli_create_address_pair_l3_subnet_no_mac(self):
        ...
        vsd_l3_domain, vsd_l3_subnet = self._given_vsd_l3subnet(cidr4=cidr4, cidr6=cidr6 )
    ...
    def test_cli_create_address_pair_l3_subnet(self):
        # Given I have a VSD-L3-Managed subnet
        vsd_l3_domain, vsd_l3_subnet = self._given_vsd_l3subnet(cidr4=self.cidr4, cidr6=self.cidr6 )
        cli_network, cli_subnet4, cli_subnet6 = self._cli_create_os_l2_vsd_managed_dualstack_subnet(vsd_l3_subnet)
