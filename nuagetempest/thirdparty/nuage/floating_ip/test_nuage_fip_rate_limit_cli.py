...
class TestNuageFipRateLimitCliWithoutDefault(remote_cli_base_testcase.RemoteCliBaseTestCase):
    ...
    def _verify_fip_openstack(self, port, created_floating_ip, default_rate_limit):
        ...
        self.assertEqual(created_floating_ip['nuage_fip_rate'], str(default_rate_limit))
    ...
    def _update_fip_rate_limit(self, port, floatingip_id, new_fip_rate):
        self.update_floating_ip_with_args(floatingip_id, '--nuage-fip-rate', str(new_fip_rate))
        ...
        self._verify_fip_vsd(port, updated_floating_ip, openstack_to_vsd(new_fip_rate))
    ...
    def test_create_fip_without_rate_limit(self):
        ...
        # Then I got a valid OpenStack FIP with the default rate limit
        self._verify_fip_openstack(port, created_floating_ip, self.expected_default_fip_rate)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, created_floating_ip, openstack_to_vsd(self.expected_default_fip_rate))
    ...
    def test_create_update_fip_with_rate_limit_normal_value(self):
        ...
        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, created_floating_ip, str(rate_limit))

        # Update value
        updated_rate_limit = 856
        self._update_fip_rate_limit(port, created_floating_ip['id'], updated_rate_limit)


class TestNuageFipRateLimitCliWithDefaultUnlimited(TestNuageFipRateLimitCliWithoutDefault):
    configured_default_fip_rate = constants.UNLIMITED
    expected_default_fip_rate = constants.UNLIMITED


class TestNuageFipRateLimitCliWithDefault(TestNuageFipRateLimitCliWithoutDefault):
    ...
    def test_create_fip_with_default_rate_limit_unlimited(self):
        ...
        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, created_floating_ip, "INFINITY")
    ...
    def test_create_update_fip_rate_limit_with_keyword_default(self):
        ...
        # ################################
        rate_limit = -1
        self._update_fip_rate_limit(port, created_floating_ip['id'], rate_limit)
        ...
        # ################################
        rate_limit = 'default'
        self.update_floating_ip_with_args(created_floating_ip['id'], '--nuage-fip-rate', str(rate_limit))
        updated_floating_ip = self.show_floating_ip(created_floating_ip['id'])

        # Then I got a valid OpenStack FIP with the default rate limit
        self._verify_fip_openstack(port, updated_floating_ip, self.expected_default_fip_rate)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, updated_floating_ip, openstack_to_vsd(self.expected_default_fip_rate))
