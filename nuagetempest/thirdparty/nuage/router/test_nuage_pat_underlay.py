...
class TestNuagePatUnderlay(base_nuage_pat_underlay.NuagePatUnderlayBase):
    ...
    @nuage_test.header()
    def test_show_router_without_eternal_gateway(self):
    ...
    def test_create_router_with_snat_invalid_neg(self):
        ...
        enable_snat_states = ['Ttrue', 'Treu', 'Tru', 'Truet', 'Trrue', 'Truue', 'Truee',
    ...
    @nuage_test.header()
    def test_create_router_with_gateway_with_non_existing_ext_network_neg(self):
        """
        Try to create router with external gateway with a non-existing external network uuid

        Must fail
        """
        name = data_utils.rand_name('router-with-external-gateway-non-existing-networkid')
    ...
    def test_create_router_with_external_gw_with_vsd_managed_subnet_neg(self):
        ...
        self.addCleanup(self.admin_networks_client.delete_network, ext_network['id'])
        # Create the external FIP subnet with underlay and referring to the VSD managed subnet
        # Must fail as VSD managed subnets cannot be linked to an external network
    ...
    def test_create_router_with_internal_network_neg(self):
        """
        Try to create router with external gateway with an internal network uuid

        Must fail
        """
        network_name = name = data_utils.rand_name('pat-int-network')
        int_network = self.create_network(network_name=network_name)
        name = data_utils.rand_name('router-with-external-gateway-internal-network')
        external_gateway_info = {
    ...
    def test_add_subnet_to_existing_pat_router_neg(self):
        ...
        for enable_snat in enable_snat_states:
            ...
            external_gateway_info = {
                'network_id': ext_network['id'],
                'enable_snat': enable_snat}
            # create_body = self.create_router(router_name=router_name,
            #                                     admin_state_up=True,
