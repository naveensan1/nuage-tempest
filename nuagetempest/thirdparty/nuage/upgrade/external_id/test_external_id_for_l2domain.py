...
class ExternalIdForL2domainTest(base.BaseNetworkTest):
    ...
    class MatchingVsdL2domain():
        ...
        def has_l2domain_template(self, with_external_id=None):
            ...
            vsd_l2domain_templates = self.test.nuage_vsd_client.get_l2domaintemplate(
                filters='name', filter_value=self.subnet['id'])
            self.test.assertEqual(len(vsd_l2domain_templates), 1, "vsd_l2domain_template not found by parent ID")

            if with_external_id is None:
                self.test.assertIsNone(vsd_l2domain_templates[0]['externalID'])
            else:
                # has external ID
                self.test.assertEqual(with_external_id, vsd_l2domain_templates[0]['externalID'])

                # can find vsd permissions by external ID
                vsd_l2domain_templates = self.test.nuage_vsd_client.get_l2domaintemplate(
                    filters='externalID', filter_value=with_external_id)
                self.test.assertEqual(len(vsd_l2domain_templates), 1, "vsd_l2domain_template not found by ExternalID")
        ...
        def has_permissions(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(with_external_id, vsd_permissions[0]['externalID'])
        ...
        def has_group(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(with_external_id, vsd_groups[0]['externalID'])
        ...
        def has_user(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(with_external_id, vsd_users[0]['externalID'])
        ...
        def has_dhcp_options(self, with_external_id=None, with_dhcp_opts=None):
            ...
            if with_external_id is None:
                ...
                for vsd_dhcp_option in vsd_dhcp_options:
                    self.test.assertEqual(with_external_id, vsd_dhcp_option['externalID'])
        ...
        def has_egress_acl_template(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(with_external_id, vsd_egress_acl_templates[0]['externalID'])
        ...
        def has_ingress_acl_template(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(with_external_id, vsd_ingress_acl_templates[0]['externalID'])

        def has_forwarding_policy_template(self, with_external_id=None):
            # vsd forwarding_policy_template object has external ID
            vsd_forwarding_policy_templates = self.test.nuage_vsd_client.get_child_resource(
                resource=n_constants.L2_DOMAIN,
                resource_id=self.vsd_l2domain['ID'],
                child_resource=n_constants.INGRESS_ADV_FWD_TEMPLATE)

            self.test.assertEqual(len(vsd_forwarding_policy_templates), 1,
                                  "forwarding_policy_template not found by VSD parent ID")

            if with_external_id is None:
                self.test.assertIsNone(vsd_forwarding_policy_templates[0]['externalID'])
            else:
                vsd_forwarding_policy_templates = self.test.nuage_vsd_client.get_child_resource(
                    resource=n_constants.L2_DOMAIN,
                    resource_id=self.vsd_l2domain['ID'],
                ...
                self.test.assertEqual(with_external_id, vsd_forwarding_policy_templates[0]['externalID'])
    ...
    def test_neutron_isolated_subnet_matches_to_l2domain(self):
        ...
        if self.test_upgrade:
            ...
            vsd_l2domain.has_dhcp_options(with_external_id=None, with_dhcp_opts=dhcp_opts)
        ...
        vsd_l2domain = self.MatchingVsdL2domain(self, subnet).get_by_external_id()
        vsd_l2domain.has_l2domain_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
        vsd_l2domain.has_dhcp_options(with_external_id=ExternalId(subnet['id']).at_cms_id(), with_dhcp_opts=dhcp_opts)
        vsd_l2domain.has_permissions(with_external_id=ExternalId(subnet['tenant_id']).at_cms_id())
        vsd_l2domain.has_group(with_external_id=ExternalId(subnet['tenant_id']).at_openstack())
        vsd_l2domain.has_user(with_external_id=ExternalId(subnet['tenant_id']).at_openstack())
        vsd_l2domain.has_egress_acl_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
        vsd_l2domain.has_ingress_acl_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
        vsd_l2domain.has_forwarding_policy_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
    ...
    def test_neutron_isolated_subnet_in_netpartition(self):
        ...
        self.addCleanup(self.nuage_network_client.delete_netpartition, netpartition_a['id'])
        ...
        subnet_a1 = self.create_subnet(network_a1, net_partition=netpartition_a['name'])
        ...
        subnet_a2 = self.create_subnet(network_a2, net_partition=netpartition_a['name'])
        ...
        subnet_b1 = self.create_subnet(network_b1, net_partition=netpartition_b['name'])

        if self.test_upgrade:
            vsd_l2domain_a1 = self.MatchingVsdL2domain(self, subnet_a1).get_by_external_id(
            ...
            vsd_l2domain_b1 = self.MatchingVsdL2domain(self, subnet_b1).get_by_external_id(
        ...
        # has only 1 group and 1 user with same External ID as subnet in netpartition A
        vsd_l2domain_a1 = self.MatchingVsdL2domain(self, subnet_a1).get_by_external_id(
            net_partition_name=netpartition_a['name'])
        vsd_l2domain_a1.has_group(with_external_id=ExternalId(subnet_a1['tenant_id']).at_openstack())
        vsd_l2domain_a1.has_user(with_external_id=ExternalId(subnet_a1['tenant_id']).at_openstack())

        # has group and user with same External ID as subnet in netpartition A
        vsd_l2domain_b1 = self.MatchingVsdL2domain(self, subnet_b1).get_by_external_id(
            net_partition_name=netpartition_b['name'])
        vsd_l2domain_b1.has_group(with_external_id=ExternalId(subnet_a1['tenant_id']).at_openstack())
        vsd_l2domain_b1.has_user(with_external_id=ExternalId(subnet_a1['tenant_id']).at_openstack())
...
class ExternalIdForL2domainAdminTest(ExternalIdForL2domainTest):
    ...
    def test_neutron_isolated_shared_subnet_matches_to_l2domain(self):
        ...
        body = self.os_adm.networks_client.create_network(name=name, shared=True)
        ...
        # Create a subnet
        subnet = self.create_subnet(network,
                                    client=self.os_adm.subnets_client)

        # Create a second subnet
        subnet_b = self.create_subnet(network, client=self.os_adm.subnets_client)

        if self.test_upgrade:
            vsd_l2domain_a = self.MatchingVsdL2domain(self, subnet).get_by_external_id()
            ...
            vsd_l2domain_a.has_forwarding_policy_template(with_external_id=None)

            vsd_l2domain_b = self.MatchingVsdL2domain(self, subnet_b).get_by_external_id()
            ...
            vsd_l2domain_b.has_forwarding_policy_template(with_external_id=None)

            upgrade_script.do_run_upgrade_script()

        vsd_l2domain_a = self.MatchingVsdL2domain(self, subnet).get_by_external_id()
        vsd_l2domain_a.has_permissions(with_external_id=ExternalId(subnet['tenant_id']).at_cms_id())
        vsd_l2domain_a.has_group_everybody()
        vsd_l2domain_a.has_user(with_external_id=ExternalId(subnet['tenant_id']).at_openstack())
        vsd_l2domain_a.has_egress_acl_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
        vsd_l2domain_a.has_ingress_acl_template(with_external_id=ExternalId(subnet['id']).at_cms_id())
        vsd_l2domain_a.has_forwarding_policy_template(with_external_id=ExternalId(subnet['id']).at_cms_id())

        vsd_l2domain_b = self.MatchingVsdL2domain(self, subnet_b).get_by_external_id()
        vsd_l2domain_b.has_permissions(with_external_id=ExternalId(subnet_b['tenant_id']).at_cms_id())
        vsd_l2domain_b.has_group_everybody()
        vsd_l2domain_b.has_user(with_external_id=ExternalId(subnet_b['tenant_id']).at_openstack())
        vsd_l2domain_b.has_egress_acl_template(with_external_id=ExternalId(subnet_b['id']).at_cms_id())
        vsd_l2domain_b.has_ingress_acl_template(with_external_id=ExternalId(subnet_b['id']).at_cms_id())
        vsd_l2domain_b.has_forwarding_policy_template(with_external_id=ExternalId(subnet_b['id']).at_cms_id())
