...
class TestNuageBidirectionalFipRateLimit(base_nuage_bidirectional_fip_rate_limit.NuageBidirectionalFipRateLimitBase):
    """
    ...
    def test_create_floatingip_with_rate_limit_maximal_value(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], constants.MAX_INT, constants.MAX_INT)
    ...
    # message': u'Bad request: nuage_ingress_fip_rate_kbps value cannot be in fraction'
    ...
    def test_create_floatingip_with_rate_limit_unlimited_value(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], constants.UNLIMITED, constants.UNLIMITED)
    ...
    def test_update_floatingip_with_rate_limit_unlimited_value(self):
        ...
        self._update_fip_with_fip_rate_limit(self.ports[0], fip, constants.UNLIMITED, constants.UNLIMITED)

    @nuage_test.header()
    def test_update_floatingip_with_rate_limit_maximal_value(self):
        fip = self._create_fip_with_fip_rate_limit(self.ports[0], 3000, 5000)
        self._update_fip_with_fip_rate_limit(self.ports[0], fip, constants.MAX_INT, constants.MAX_INT)

    def test_update_floatingip_with_rate_limit_high_value(self):
        fip = self._create_fip_with_fip_rate_limit(self.ports[0], 3000, 5000)
        self._update_fip_with_fip_rate_limit(self.ports[0], fip, 100000, 100000)
    ...
    def test_create_floatingip_with_rate_limit_unlimited_value_ingress(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], constants.UNLIMITED)
    ...
    def test_create_floatingip_with_rate_limit_normal_value_egress(self):
        fip = self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=2000)
        self._update_fip_with_fip_rate_limit(self.ports[0], fip, egress_rate_limit=5000)

    @nuage_test.header()
    def test_show_floatingip_with_rate_limit_normal_value_egress(self):
        fip = self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=4000)
        self._show_fip_with_fip_rate_limit(self.ports[0], fip, egress_rate_limit=4000)

    @nuage_test.header()
    # OPENSTACK-1583
    def test_create_floatingip_with_rate_limit_minimal_value_egress(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit='0')

    @nuage_test.header()
    def test_create_floatingip_with_rate_limit_maximal_value_egress(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=constants.MAX_INT)

    @nuage_test.header()
    def test_create_floatingip_with_rate_limit_high_value_egress(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=100000)

    @nuage_test.header()
    def test_create_floatingip_with_rate_limit_unlimited_value_egress(self):
        self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=constants.UNLIMITED)

    @nuage_test.header()
    def test_update_floatingip_with_rate_limit_minimal_value_egress(self):
        fip = self._create_fip_with_fip_rate_limit(self.ports[0], egress_rate_limit=2000)
        self._update_fip_with_fip_rate_limit(self.ports[0], fip, egress_rate_limit=0)
