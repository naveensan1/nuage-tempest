...
class VSDManagedRedirectTargetCLITest(remote_cli_base_testcase.RemoteCliBaseTestCase,
    ...
    def test_cli_create_delete_os_redirection_target_l2_mgd_subnet(self):
        ...
        nuage_redirect_target = self._cli_create_nuage_redirect_target_in_l2_subnet(cli_subnet)
        # os_redirect_target = self._create_redirect_target_in_l2_subnet(subnet)
        # Then I expect the redirection-target in my list
        my_rt_found = self._cli_find_redirect_target_in_list(nuage_redirect_target['id'], cli_subnet)
        ...
        self.assertNotEmpty(vsd_redirect_target, "Redirect target not present on VSD")
        ...
        port_present = self._cli_check_port_in_show_redirect_target(rtport, os_redirect_target)
        ...
        port_present = self._cli_check_port_in_show_redirect_target(rtport, nuage_redirect_target)
        ...
        my_rt_found = self._cli_find_redirect_target_in_list(nuage_redirect_target, cli_subnet)
    ...
    def test_cli_create_delete_vsd_redirection_target_l2_mgd_subnet(self):
        ...
        redirect_target = self.show_nuage_redirect_target(vsd_redirect_target[0]['ID'])
        # Then I expect the redirection-target in my list
        my_rt_found = self._find_redirect_target_in_list(vsd_redirect_target[0]['ID'], cli_subnet)
        self.assertTrue(my_rt_found, "Did not find my redirect-target in the list")
        # When I associate a port to the redirectict-target
        rtport = self.create_port(cli_network)
        self._cli_associate_rt_port(rtport, redirect_target)
        # Then I expect the port in the show redirect-target response
        port_present = self._cli_check_port_in_show_redirect_target(rtport, redirect_target)
        ...
        # I expect the redirect-target to be gone from the list
        my_rt_found = self._cli_find_redirect_target_in_list(redirect_target, cli_subnet)
        self.assertEqual(False, my_rt_found,
    ...
    def test_cli_create_delete_several_redirection_targets_l2_mgd_subnet(self):
        ...
        for i in range(SEVERAL_REDIRECT_TARGETS):
            os_redirect_targets.append(self._cli_create_nuage_redirect_target_in_l2_subnet(cli_subnet))
            vsd_redirect_target = self.nuage_vsd_client.create_l2_redirect_target(vsd_l2_subnet[0]['ID'],
                                                                                  data_utils.rand_name("vsd-rt"))
        ...
        # Then I expect the redirection-target in my list
        for i in range(SEVERAL_REDIRECT_TARGETS):
            my_os_rt_found = self._cli_find_redirect_target_in_list(os_redirect_targets[i]['id'], cli_subnet)
            ...
            my_vsd_rt_found = self._cli_find_redirect_target_in_list(
                vsd_redirect_targets[i]['nuage_redirect_target']['id'], cli_subnet)
        ...
        for i in range(SEVERAL_REDIRECT_TARGETS):
            ...
            # Then I expect the port in the show redirect-target response
            port_present = self._cli_check_port_in_show_redirect_target(rtport, os_redirect_targets[i])
            self.assertTrue(port_present,
                            "Associated port not present in show nuage redirect target response")
            # When I disassociate the red0rect-target from the port        #
        ...
        for i in range(SEVERAL_REDIRECT_TARGETS):
            ...
            # I expect the redirect-target to be gone from the list
            my_rt_found = self._cli_find_redirect_target_in_list(os_redirect_targets[i], cli_subnet)
            self.assertEqual(False, my_rt_found,
                             message="Deleted nuage_redirect_target still present in subnet")
            ...
            my_vsd_rt_found = self._cli_find_redirect_target_in_list(vsd_redirect_targets[i], cli_subnet)
    ...
    def test_cli_create_delete_os_redirection_target_l3_mgd_subnet(self):
        ...
        cli_network, cli_subnet = self._cli_create_os_l3_vsd_managed_subnet(vsd_l3_subnet)
        #  When I create a redirection-target in the VSD-L3-Managed-Subnet
        os_redirect_target = self._cli_create_nuage_redirect_target_in_l3_subnet(cli_subnet)
        # Then I expect the redirection-target in my list
        my_rt_found = self._cli_find_redirect_target_in_list(os_redirect_target['id'], cli_subnet)
        self.assertTrue(my_rt_found, "Did not find my redirect-target in the list")
        # check on VSD
        vsd_redirect_target = self.nuage_vsd_client.get_redirection_target(
    ...
    def test_cli_create_delete_vsd_redirection_target_l3_mgd_subnet(self):
        ...
        vsd_redirect_target = self.nuage_vsd_client.create_l3_redirect_target(vsd_l3_domain[0]['ID'],
                                                                              data_utils.rand_name("vsd-rt"))
        # Fetch this redircet_target in OS, as this structure is used throught the test
        redirect_target = self.show_nuage_redirect_target(vsd_redirect_target[0]['ID'])
        # Then I expect the redirection-target in my list
        my_rt_found = self._cli_find_redirect_target_in_list(vsd_redirect_target[0]['ID'], cli_subnet)
        self.assertTrue(my_rt_found, "Did not find my redirect-target in the list")
        # When I associate a port to the redirectict-target
        rtport = self.create_port(cli_network)
        self._cli_associate_rt_port(rtport, redirect_target)
    ...
    def test_cli_create_delete_several_redirection_targets_l3_mgd_subnet(self):
        ...
        #  both on openstack and VSD
        for i in range(SEVERAL_REDIRECT_TARGETS):
            os_redirect_targets.append(self._cli_create_nuage_redirect_target_in_l3_subnet(cli_subnet))
            vsd_redirect_target = self.nuage_vsd_client.create_l3_redirect_target(vsd_l3_domain[0]['ID'],
        ...
        for i in range(SEVERAL_REDIRECT_TARGETS):
            # When I delete the redirect-target
            self.delete_redirect_target(os_redirect_targets[i]['id'])
            # self.nuage_network_client.delete_redirection_target(os_redirect_targets[i]['id'])
            # I expect the redirect-target to be gone from the list
            my_rt_found = self._cli_find_redirect_target_in_list(os_redirect_targets[i], cli_subnet)
            self.assertEqual(False, my_rt_found,
                             message="Deleted nuage_redirect_target still present in subnet")
            ...
            self.assertEqual(vsd_redirect_target, '')
            # When I delete the VSD created redirect-target
            self.delete_redirect_target(vsd_redirect_targets[i]['nuage_redirect_target']['id'])
            # self.nuage_vsd_client.delete_redirect_target(vsd_redirect_targets[i]['nuage_redirect_target']['id'])
            # Then I expect the redirect_target to be gone from my list
            my_vsd_rt_found = self._cli_find_redirect_target_in_list(vsd_redirect_targets[i], cli_subnet)
            self.assertEqual(False, my_vsd_rt_found,
    ...
    @nuage_test.header()
    def test_cli_create_os_redirection_target_same_name_diff_l2_mgd_subnet(self):
        # Given I have a VSD-L2-Managed-Subnet-x in openstack
        vsd_l2_subnet_x, l2dom_template_x = self._create_vsd_l2_managed_subnet()
        cli_network_x, cli_subnet_x = self._cli_create_os_l2_vsd_managed_subnet(vsd_l2_subnet_x)
        ...
        my_rt_found_x = self._cli_find_redirect_target_in_list(os_redirect_target_x['id'], cli_subnet_x)
        self.assertTrue(my_rt_found_x, "Did not find my redirect-target in the list")
        pass

    @test.attr(type=['negative'])
    @nuage_test.header()
    def test_cli_create_os_redirection_target_same_name_same_l2_mgd_subnet_neg(self):
        # Given I have a VSD-L2-Managed-Subnet in openstack
        vsd_l2_subnet, l2dom_template = self._create_vsd_l2_managed_subnet()
        cli_network, cli_subnet = self._cli_create_os_l2_vsd_managed_subnet(vsd_l2_subnet)
    ...
    def test_cli_associate_two_port_same_l2_os_redirection_target_neg(self):
        ...
        port_present = self._cli_check_port_in_show_redirect_target(rtport_1, os_redirect_target)
        self.assertTrue(port_present,
                        "Associated port not present in show nuage redirect target response")
        # When I disassociate the red0rect-target from the port        #
...
class VSDManagedPolicyGroupsCLITest(remote_cli_base_testcase.RemoteCliBaseTestCase,
                                    base_vsd_managed_port_attributes.BaseVSDManagedPortAttributes):
    ...
    def test_cli_l2_associate_port_to_policygroup(self):
        ...
        policy_group_list = self.list_nuage_policy_group_for_subnet(cli_subnet['id'])
    ...
    def test_cli_list_l2_policy_groups_subnet_only(self):
        ...
        policy_group_list_x = self.list_nuage_policy_group_for_subnet(cli_subnet_x['id'])
        ...
        self.assertFalse(pg_present, "Found policgroup (%s) of another subnet (%s) in this subnet (%s)" %
                         (policy_group_y[0]['ID'],  cli_subnet_y['id'], cli_subnet_x['id']))

        # And vice versa
        # When I retrieve the polic groups of VSD-L2-Managed-Subnet_y
        policy_group_list_y = self.list_nuage_policy_group_for_subnet(cli_subnet_y['id'])
    ...
    def test_cli_list_l3_policy_groups_subnet_only(self):
        ...
        cli_network_x, cli_subnet_x = self._cli_create_os_l3_vsd_managed_subnet(vsd_l3_subnet_x)
        ...
        cli_network_y, cli_subnet_y = self._cli_create_os_l3_vsd_managed_subnet(vsd_l3_subnet_y)
...
class VSDManagedAllowedAddresPairsCLITest(remote_cli_base_testcase.RemoteCliBaseTestCase,
                                          base_vsd_managed_port_attributes.BaseVSDManagedPortAttributes):
    ...
    def test_cli_create_address_pair_l2domain_with_mac(self):
        ...
        # And the allowed-address-pair MACaddress == port MACaddress
        show_port = self.show_port(addrpair_port['id'])
        self.cli_check_show_port_allowed_address_fields(show_port, aap_fixed_ip, VALID_MAC_ADDRESS)
        # And no corresponding MultiVIP on the VSD
        port_ext_id = self.nuage_vsd_client.get_vsd_external_id(addrpair_port['id'])
        nuage_vport = self.nuage_vsd_client.get_vport(
            constants.L2_DOMAIN,
    ...
    def test_cli_create_address_pair_l3_subnet_no_mac(self):
        ...
        # And the allowed-address-pair MACaddress == port MACaddress
        show_port = self.show_port(addrpair_port['id'])
        self.cli_check_show_port_allowed_address_fields(show_port,
                                                        aap_fixed_ip,
                                                        addrpair_port['mac_address'])
        # And no corresponding MultiVIP on the VSD
        port_ext_id = self.nuage_vsd_client.get_vsd_external_id(addrpair_port['id'])
...
class VSDManagedAssociateFIPCLITest(remote_cli_base_testcase.RemoteCliBaseTestCase,
    ...
    #         cls.nuage_vsd_client.delete_vsd_fpolicygroup(vsd_policy_group[0]['id'])
    ...
    def test_cli_create_list_associate_vsd_floatingip(self):
        ...
        fip_present = self.cli_check_fip_in_list(claimed_fip[0]['ID'], fip_list)
        ...
        if CONF.nuage_sut.nuage_plugin_mode != 'ml2':
            fip_present = self.cli_check_fip_in_port_show(claimed_fip[0]['ID'], port['id'])
    ...
    def test_cli_create_list_associate_several_vsd_floatingip(self):
        ...
        for i in range(SEVERAL_VSD_CLAIMED_FIPS):
            fip_present = self.cli_check_fip_in_list(claimed_fips[i][0]['ID'], fip_list)
            self.assertTrue(fip_present, msg="nuage floatingip not present in list, while expected to be")
        ...
        # And I associate this port to the claimed floating ip (via update)
        for i in range(SEVERAL_VSD_CLAIMED_FIPS):
            self.cli_associate_fip_to_port(claimed_fips[i][0]['ID'], ports[i]['id'])
        for i in range(SEVERAL_VSD_CLAIMED_FIPS):
            # Then I expect the claimed floating ip in the port show response
            if CONF.nuage_sut.nuage_plugin_mode != 'ml2':
                fip_present = self.cli_check_fip_in_port_show(claimed_fips[i][0]['ID'], ports[i]['id'])
                self.assertTrue(fip_present,
                                msg="associated VSD claimed FIP (%s) not found in port (%s)" %
    ...
    def test_cli_subnets_same_domain_associate_vsd_floatingip(self):
        ...
        cli_network_y, cli_subnet_y = self._cli_create_os_l3_vsd_managed_subnet(
            vsd_l3_subnet_y,
        ...
        fip_present_x = self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_x)
        self.assertTrue(fip_present_x, msg="nuage floatingip not present in list, while expected to be")
        fip_present_y = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_x)
        ...
        # I expect both VSD-floatingIP's in the list
        fip_present = self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_y)
        self.assertTrue(fip_present, msg="nuage floatingip not present in list, while expected to be")
        fip_present = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_y)
        self.assertTrue(fip_present, msg="nuage floatingip not present in list, while expected to be")
        ...
        fip_list_y = self.list_nuage_floatingip_by_subnet(cli_subnet_y['id'])
        fip_present_x = self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_y)
        self.assertFalse(fip_present_x,
                         msg="associated VSD claimed FIP (%s) still found as available in subnet-list (%s)" %
        ...
        fip_list_y = self.list_nuage_floatingip_by_subnet(cli_subnet_y['id'])
        fip_present_y = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_y)
        ...
        fip_list_y = self.list_nuage_floatingip_by_subnet(cli_subnet_y['id'])
        fip_present_y = self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_y)
        self.assertTrue(fip_present_y, msg="nuage floatingip not present in list, while expected to be")
        ...
        fip_list_x = self.list_nuage_floatingip_by_subnet(cli_subnet_x['id'])
        fip_present_x = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_x)
        self.assertTrue(fip_present_x, msg="nuage floatingip not present in list, while expected to be")
        # And is is also available in the list of subnet-Y
        fip_list_y = self.list_nuage_floatingip_by_subnet(cli_subnet_y['id'])
        fip_present_y = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_y)
    ...
    def test_cli_subnets_other_domain_associate_vsd_floatingip(self):
        ...
        self.assertTrue(self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_x),
                        msg="nuage floatingip not present in list, while expected to be")
        fip_present = self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_x)
        ...
        fip_list_x = self.list_nuage_floatingip_by_subnet(cli_subnet_x['id'])
        self.assertFalse(self.cli_check_fip_in_list(claimed_fip_x[0]['ID'], fip_list_x),
                         msg="nuage floatingip not present in list, while expected to be")
        ...
        fip_list_y = self.list_nuage_floatingip_by_subnet(cli_subnet_y['id'])
        self.assertFalse(self.cli_check_fip_in_list(claimed_fip_y[0]['ID'], fip_list_y),
                         msg="nuage floatingip not present in list, while expected to be")
    ...
    def test_create_associate_vsd_floatingip_twice_neg(self):
        ...
        # Then I expect the claimed floating ip in the port show response
        if CONF.nuage_sut.nuage_plugin_mode != 'ml2':
            fip_present = self.cli_check_fip_in_port_show(claimed_fip[0]['ID'], port_1['id'])
            self.assertTrue(fip_present,
                            msg="associated VSD claimed FIP (%s) not found in port (%s)" %
