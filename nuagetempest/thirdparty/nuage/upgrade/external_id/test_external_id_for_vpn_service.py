...
class ExternalIdForVpnServiceTest(VPNMixin, base.BaseNetworkTest):
    class MatchingVsdVpnServiceObjects():
        ...
        def has_dummy_router(self, with_external_id=None):
            ...
            self.test.assertEqual(len(vsd_dummy_routers), 1, "Dummy router not found by VSD domain description")
        ...
        def has_floating_ip(self, with_external_id=None):
            ...
            self.test.assertEqual(len(vsd_floating_ips), 1, "VPN Floating IP router not found by IP address")
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(len(vsd_floating_ips), 1, "VPN Floating IP router not found by External ID")
        ...
        with self.vpnservice(router_a1['id'], subnet_a1['id'],
            ...
            with self.ipsecsiteconnection(
                ...
                if self.test_upgrade:
                    vpn_service_match = self.MatchingVsdVpnServiceObjects(self, created_vpnservice)
                ...
                vpn_service_match = self.MatchingVsdVpnServiceObjects(self, created_vpnservice)
                vpn_service_match.has_dummy_router(with_external_id=None)
                vpn_service_match.has_floating_ip(with_external_id=ExternalId(created_vpnservice['id']).at_cms_id())
