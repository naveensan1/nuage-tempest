...
class NuageBidirectionalFipRateLimitBase(base.BaseNetworkTest):
    ...
    @classmethod
    def _create_fip_for_port_with_rate_limit(cls, port_id, ingress_rate_limit=None, egress_rate_limit=None):
        rate_limit_dict = {}
    ...
    def _do_create_fip_for_port_with_rate_limit(self, port_id, ingress_rate_limit=None, egress_rate_limit=None):
        rate_limit_dict = {}
        if ingress_rate_limit is not None:
            rate_limit_dict['nuage_ingress_fip_rate_kbps'] = ingress_rate_limit
        if egress_rate_limit is not None:
    ...
    def _do_create_fip_for_port_with_rate_limit_backward(self, port_id, rate_limit=None):
        rate_limit_dict = {}
    ...
    def _do_update_fip_to_rate_limit(self, floating_ip_id, ingress_rate_limit=None, egress_rate_limit=None):
        rate_limit_dict = {}
        if ingress_rate_limit is not None:
            rate_limit_dict['nuage_ingress_fip_rate_kbps'] = ingress_rate_limit
        if egress_rate_limit is not None:
    ...
    def _verify_fip_openstack(self, port, created_floating_ip, ingress_rate_limit=None, egress_rate_limit=None, backward=False):
    ...
    def _verify_fip_vsd(self, port, created_floating_ip, ingress_rate_limit=None, egress_rate_limit=None, backward=False):
    ...
    def _create_fip_with_fip_rate_limit(self, port, ingress_rate_limit=None, egress_rate_limit=None):
        # When I create a fip with default rate limit
        
        created_floating_ip = self._do_create_fip_for_port_with_rate_limit(port['id'], ingress_rate_limit, egress_rate_limit)
        ...
        self._verify_fip_openstack(port, show_floating_ip, ingress_rate_limit, egress_rate_limit)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, created_floating_ip, openstack_to_vsd(ingress_rate_limit), openstack_to_vsd(egress_rate_limit))
    ...
    def _create_fip_with_fip_rate_limit_backward(self, port, rate_limit=None):
        ...
        created_floating_ip = self._do_create_fip_for_port_with_rate_limit_backward(port['id'], rate_limit)

        show_floating_ip = self._do_get_floating_ip(created_floating_ip['id'])
        # Then I got a valid OpenStack FIP with the default rate limit
        self._verify_fip_openstack(port, show_floating_ip, None, rate_limit, backward=True)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, created_floating_ip, openstack_to_vsd(None), openstack_to_vsd(rate_limit), backward=True)

        return created_floating_ip

    def _update_fip_with_fip_rate_limit(self, port, floating_ip, ingress_rate_limit=None, egress_rate_limit=None):
        # When I create a fip with default rate limit
        updated_floating_ip = self._do_update_fip_to_rate_limit(floating_ip['id'], ingress_rate_limit, egress_rate_limit)
    ...
    def _show_fip_with_fip_rate_limit(self, port, floating_ip, ingress_rate_limit=None, egress_rate_limit=None):
        # When I create a fip with default rate limit
        floating_ip = self._do_get_floating_ip(floating_ip['id'])

        # Then I got a valid OpenStack FIP with the default rate limit
        self._verify_fip_openstack(port, floating_ip, ingress_rate_limit, egress_rate_limit)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, floating_ip, openstack_to_vsd(ingress_rate_limit), openstack_to_vsd(egress_rate_limit))
