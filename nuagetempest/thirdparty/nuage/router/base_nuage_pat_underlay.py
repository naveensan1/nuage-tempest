...
class NuagePatUnderlayBase(base.BaseRouterTest):
    service_manager = None
    _interface = 'json'

    LOG = logging.getLogger(__name__)
    PAT_NEEDS_EXT_NETWORK = "Invalid input for external_gateway_info. Reason: Validation of dictionary's keys failed."
    PAT_NOTAVAILABLE_EXT_GW_INFO = "nuage_pat config is set to 'not_available'." \
    ...
    def _verify_create_router_without_external_gateway(self):
        """
           Create router without external gateway, nuage_pat = self.nuage_pat_ini
        ...
        nuage_domain = self.nuage_vsd_client.get_l3domain(
            filters='externalID',
            filter_value=self.nuage_vsd_client.get_vsd_external_id(create_body['router']['id']))
        self.assertEqual(
    ...
    def _verify_create_router_with_external_gw_without_snat(self):
        ...
        """
        name = data_utils.rand_name('router-with-ext-gw-without-snat-') + str(self.nuage_pat_ini)
    ...
    def _verify_create_router_without_external_gateway_with_snat_neg(self):
        """
        Create router without external gateway, but specify enable_snat (True/False)

        Must fail with 'BadRequest', as enable_snat is only for external gateway routers
        """
        name = data_utils.rand_name('router-without-ext-gw-with-snat-' + str(self.nuage_pat_ini))
    ...
    def _verify_create_router_with_external_gateway_with_snat(self):
        """
        Create router with external gateway and specifying enable_snat explicitly

        Response must include the same enable_snat value as in the request
        VSD patEnabled flag must be Disabled/Enabled according enable_snat value
        This test must succeed for nuage_pat = DefaultDisabled and DefaultEnabled
        """
        name = data_utils.rand_name('router-with-ext-gw-with-snat-' + str(self.nuage_pat_ini))
        ext_network = self._create_ext_network()
        ...
        for enable_snat in enable_snat_states:
            ...
            self.assertEqual(
                nuage_domain[0]['PATEnabled'],
                constants.NUAGE_PAT_VSD_ENABLED if enable_snat else constants.NUAGE_PAT_VSD_DISABLED)
    ...
    def _verify_update_router_with_gateway_with_snat(self):
        ...
        """
        name = data_utils.rand_name('update-router-with-ext-gw-with-snat-' + str(self.nuage_pat_ini))
        ext_network = self._create_ext_network()
        enable_snat_states = [False, True]
        for enable_snat in enable_snat_states:
            external_gateway_info = {
            ...
            self.admin_client.update_router_with_snat_gw_info(
                create_body['router']['id'], external_gateway_info=updated_external_gateway_info)
            ...
            self.assertEqual(
                nuage_domain[0]['PATEnabled'],
                constants.NUAGE_PAT_VSD_DISABLED if enable_snat else constants.NUAGE_PAT_VSD_ENABLED)
    ...
    def _verify_show_router_with_external_gateway_with_snat(self):
        ...
        """
        name = data_utils.rand_name('show-router-with-ext-gw-with-snat-' + str(self.nuage_pat_ini))
        ext_network = self._create_ext_network()
        enable_snat_states = [False, True]
        for enable_snat in enable_snat_states:
            external_gateway_info = {
    ...
    def _verify_list_router_with_gateway_with_snat(self):
        ...
        my_router_list = ['router-true', 'router-false', 'router-false', 'router-true']
        ext_network = self._create_ext_network()
        for create_router in my_router_list:
            name = data_utils.rand_name(create_router)
            # set enable_snat=true/false according the true/false part in the router-name
            if re.search('true', create_router):
                enable_snat = True
            elif re.search('false', create_router):
                enable_snat = False
            else:
                # use this to check the default_disabled/default_enabled behavior
                enable_snat = None
            ...
            for router in list_body['routers']:
                ...
                if created_id == listed_id:
                    ...
                    if enable_snat is not None:
                        compare_snat = create_body['router']['external_gateway_info']['enable_snat']
                        listed_snat = router['external_gateway_info']['enable_snat']
                        self.assertEqual(compare_snat, listed_snat,
    ...
    def _cli_create_router_without_external_gateway_neg(self):
        ...
        Must succeed and no "external+_gateway_info" section may be present in the response
        """
        # When I create a router without external gateway info
        self.router = self.create_router()
        # Then I expect the response to contain an empty "external_gateway_info" section
        self.assertEmpty(self.router['external_gateway_info'],
                         "PAT 2 underlay: external_gateway_info section is not empty, while it must be")

    def _cli_create_router_without_external_gateway_with_snat_neg(self):
        """
        Via CLI, Create router without external gateway, but specify enable_snat (True/False)

        Must fail, as enable_snat is only for external gateway routers
        """
        name = data_utils.rand_name('pat-router-without-ext-gw-with-snat-' + str(self.nuage_pat_ini))
        enable_snat_states = [False, True]
        for enable_snat in enable_snat_states:
            external_gateway_info_cli = '--external_gateway_info type=dict enable_snat=' + str(enable_snat)
            # When I create a router and specify external_gateway-info enable_snat, without a network
    ...
    def _cli_create_router_with_external_gateway_without_snat(self):
        ...
        Router is created, enable_snat depends on the nuage_pat setting in the ini file
        empty / default_disabled  -> enable_snat = true, VSD: false
        default_enabled            -> enable_snat = true, VSD: True
        """
        network_name = data_utils.rand_name('ext-pat-network-' + str(self.nuage_pat_ini))
        router_name = data_utils.rand_name('pat-router-' + str(self.nuage_pat_ini))
        self.network = self.create_network_with_args(network_name, ' --router:external')
        external_gateway_info_cli = '--external_gateway_info type=dict network_id=' + self.network['id']
        self.router = self.create_router_with_args(router_name, external_gateway_info_cli)
        # When I create a router with an external network and do not specify enable_snat
        if self.nuage_pat_ini == constants.NUAGE_PAT_DEFAULTENABLED:
        ...
        # I expect the router to be created and the enable snat value according the nuage_pat .ini settings
        ...
        nuage_domain = self.nuage_vsd_client.get_l3domain(
            filters='externalID',
            filter_value=self.nuage_vsd_client.get_vsd_external_id(self.router['id']))
        self.assertEqual(
            nuage_domain[0]['PATEnabled'],
            vsd_flag)
    ...
    def _cli_verify_create_router_with_external_gateway_with_snat(self):
        ...
        for enable_snat in enable_snat_states:
            ...
            self.router = self.create_router_with_args(router_name, external_gateway_info_cli)
            print "self.router.external_gateway_info =" + self.router['external_gateway_info']
    ...
    def _cli_update_router_with_external_gateway_with_snat(self):
        ...
        for enable_snat in enable_snat_states:
            network_name = data_utils.rand_name('ext-pat-network-' + str(self.nuage_pat_ini))
            self.network = self.create_network_with_args(network_name, ' --router:external')
            router_name = data_utils.rand_name('pat-update-router-' + str(self.nuage_pat_ini))
            external_gateway_info_cli = '--external_gateway_info type=dict network_id=' + \
                                        self.network['id'] + ',enable_snat=' + str(enable_snat)
            self.router = self.create_router_with_args(router_name, external_gateway_info_cli)
            compare_snat_str = '"enable_snat": ' + str(enable_snat)
            ...
            new_external_gateway_info_cli = '--external_gateway_info type=dict network_id=' + \
                                            self.network['id'] + ',enable_snat=' + str(new_enable_snat)
            self.update_router_with_args(self.router['id'], new_external_gateway_info_cli)
            ...
            self.assertEqual(
                nuage_domain[0]['PATEnabled'],
                constants.NUAGE_PAT_VSD_ENABLED if new_enable_snat else constants.NUAGE_PAT_VSD_DISABLED)
    ...
    def _cli_show_router_without_external_gw(self):
        ...
        # When I create a router without external gateway info
        self.router = self.create_router()
        # Then I expect the show response to contain a "external_gateway_info section equal to 'null'
        show_router = self.show_router(self.router['id'])
        self.assertEmpty(show_router['external_gateway_info'],
                         "PAT 2 underlay: show-router: external_gateway_info section is not empty, while it must be")
        # self.addCleanup(self.delete_router, show_router['id'])
        pass

    def _cli_show_router_with_external_gw_without_snat(self):
        """
        Show a router with external gateway created without specifying enable_snat

        Response must include the enable_snat value according the setting in the .ini file
        """
        network_name = data_utils.rand_name('ext-pat-network-' + str(self.nuage_pat_ini))
        router_name = data_utils.rand_name('pat-router-' + str(self.nuage_pat_ini))
        self.network = self.create_network_with_args(network_name, ' --router:external')
        ...
        self.router = self.create_router_with_args(router_name, external_gateway_info_cli)
        show_router = self.show_router(self.router['id'])
        # The expected enable_snat value depends on the nuage+pat setting in the .ini file
        if self.nuage_pat_ini == constants.NUAGE_PAT_DEFAULTENABLED:
            compare_snat_str = '"enable_snat": " true'
        else:
    ...
    def _cli_list_router_without_gateway(self):
        ...
        """
        my_router_list = ['pat-list-router-wo-gw-1', 'pat-list-router-wo-gw-2', 'pat-list-router-wo-gw-3']
        for create_router in my_router_list:
            ...
            for router in router_list:
                ...
                if created_id == listed_id:
                    ...
                    self.assertEqual(listed_snat, 'null',
    ...
    def _cli_list_router_with_gateway_with_snat(self):
        ...
        """
        # createdRouterList = list()
        # check for enable_snat = true / false / not given (check default behavior)
        my_router_list = ['list-router-with_snat-true', 'list-router-with-snat-false', 'list-router-with-snat-none']
        for create_router in my_router_list:
            router_name = data_utils.rand_name(create_router)
            # set enable_snat=true/false according the true/false part in the router-name
            if re.search('true', create_router):
            ...
            if enable_snat is None:
                ...
                external_gateway_info_cli = '--external_gateway_info type=dict network_id=' + \
                                            CONF.network.public_network_id + ',enable_snat=' + str(enable_snat)
            self.router = self.create_router_with_args(router_name, external_gateway_info_cli)
            router_list = self.parser.listing(self.list_routers())
            for router in router_list:
                created_id = self.router['id']
                listed_id = router['id']
                if created_id == listed_id:
                    # this is ours, check enable_snat: according value given at creation or if not given at creation
                    ...
                    nuage_domain = self.nuage_vsd_client.get_l3domain(
                        filters='externalID',
                        filter_value=self.nuage_vsd_client.get_vsd_external_id(created_id))
                    self.assertEqual(
                        nuage_domain[0]['PATEnabled'],
                        expected_vsd_pat)
        pass
    ...
    def _cli_add_subnet_to_existing_external_gateway_with_snat(self):
        ...
        # Avoid overlap of subnets when running the whole class (cleanup at class level, iso test level
        ...
        for enable_snat in enable_snat_states:
            ...
            self.subnet = self.create_subnet_with_args(network_name, str(cidr.cidr), "--name ", subnet_name)
            cidr = cidr.next(1)
        pass
    ...
    def _cli_tenant_create_router_with_external_gateway(self):
        ...
        for enable_snat in enable_snat_states:
            ...
            name = data_utils.rand_name('pat-router-' + str(self.nuage_pat_ini))
            external_gateway_info_cli = '--external_gateway_info type=dict network_id=' + \
