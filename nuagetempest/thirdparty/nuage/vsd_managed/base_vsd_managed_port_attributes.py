...
class BaseVSDManagedPortAttributes(base_vsd_managed_networks.BaseVSDManagedNetwork):
...
############################################################################
    def _cli_create_os_l2_vsd_managed_subnet(self, vsd_l2_subnet):
        ...
        net_partition = CONF.nuage.nuage_default_netpartition
        nuagenet = vsd_l2_subnet[0]['ID']
        subnet = self.create_subnet_with_args(network['name'],cidr,
                                              "--name ", subnet_name,
    ...
    def _cli_create_os_l3_vsd_managed_subnet(self,vsd_l3_subnet, cidr=None):
        # network = self.create_network(network_name=data_utils.rand_name('osl3network'))
        network = self.create_network_with_args(data_utils.rand_name('cli-osl3network'))
        if cidr is None:
            cidr = base_vsd_managed_networks.VSD_L3_SHARED_MGD_CIDR
        else:
            cidr = cidr
    ...
    def _cli_associate_rt_port(self, rtport, rt):
        self.update_port_with_args(rtport['id'], "--nuage-redirect-targets", rt['id'])

    def _cli_disassociate_rt_port(self, rtport, rt):
        self.update_port_with_args(rtport['id'], "--nuage-redirect-targets None")
    ...
    def _cli_associate_multiple_rt_port(self, rtport, rts):
        ...
        # convert into comaa separated string
        rt_string = ",".join(nuage_rt_id_list)
        self.update_port_with_args(rtport['id'], "--nuage-redirect-targets", rt_string)
    ...
    def cli_associate_port_with_policy_group(self, port, policy_group):
        self.update_port_with_args(port['id'],
                                   "--nuage-policy-groups",policy_group[0]['ID'],
                                   "--name port-with-vsd-pg")

    def cli_associate_port_with_multiple_policy_group(self, port, policy_group_id_list):
        cli_args = ''
