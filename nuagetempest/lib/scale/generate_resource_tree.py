...
class UserResourceData(object):
    ...
    def check_process_root(self, **kwargs):
        ...
        try:
            ...
            self.root.user_data = self.root.user_data if self.root.user_data else CMS()
        except Exception:
    ...
    def check_if_type_present(self, parent, type):
        '''Return number of type object children
        ...
        try:
            ...
            LOG.debug('No {} currently present in tree'.format(type))
        LOG.debug("{} Children currently present in tree".format(len(children)))
        ...
        for child in children:
            ...
            if child.user_data.type == type:
                ...
                offset = re.compile(r'(\d+)$').search(child.user_data.name).group(1)
    ...
    def check_user_domains_in_enterprise(self, enterprise):
        ...
        LOG.debug("{} Domains currently present in Enterprise {}".format(num, enterprise))
        return num, offset
    ...
    def check_user_l2domains_in_enterprise(self, enterprise):
        ...
        LOG.debug("{} L2Domains currently present in Enterprise {}".format(num, enterprise))
        return num, offset
    ...
    def check_user_zones_in_domain(self, domain):
        ...
        LOG.debug("{} Zones currently present in Domain {}".format(num, domain))
        return num, offset
    ...
    def populate_user_subnets_in_tree(self, parent, gparent, ggparent, offset, **kwargs):
    ...
    def check_endpoints_in_l2domain(self, l2domain):
        ...
        LOG.debug("{} Endpoints currently present in L2domain {}".format(num, l2domain))
        return num, offset

    def check_endpoints_in_subnet(self, subnet):
        num, offset = self.check_if_type_present(subnet, 'EndPoint')
        LOG.debug("{} Endpoints currently present in subnet {}".format(num, subnet))
        return num, offset
    ...
    def populate_user_endpoint_in_l2dom_tree(self, parent, gparent, offset, **kwargs):
    ...
    def trigger_populate_tree(self, dataStruct=None, **kwargs):
        ...
        for enterprise in self.return_children_of_type('CMS', "Enterprise"):
            num1, offset1 = self.check_user_domains_in_enterprise(enterprise.user_data.name)
            self.populate_user_domains_in_tree(enterprise.user_data.name, offset1, **kwargs)
            num1, offset1 = self.check_user_l2domains_in_enterprise(enterprise.user_data.name)
            self.populate_user_l2domains_in_tree(enterprise.user_data.name, offset1, **kwargs)
            for l2domain in self.return_children_of_type(enterprise.user_data.name, "L2Domain"):
                num4, offset4 = self.check_endpoints_in_l2domain(l2domain.user_data.name)
                self.populate_user_endpoint_in_l2dom_tree(l2domain.user_data.name, enterprise.user_data.name, offset4, **kwargs)

            for domain in self.return_children_of_type(enterprise.user_data.name, "Domain"):
                num2, offset2 = self.check_user_zones_in_domain(domain.user_data.name)
                self.populate_user_zones_in_tree(domain.user_data.name, enterprise.user_data.name, offset2, **kwargs)
                for zone in self.return_children_of_type(domain.user_data.name, "Zone"):
                    num3, offset3 = self.check_user_subnets_in_zone(zone.user_data.name)
                    self.populate_user_subnets_in_tree(zone.user_data.name, domain.user_data.name,
                                                        enterprise.user_data.name, offset3, **kwargs)
                    for subnet in self.return_children_of_type(zone.user_data.name, "Subnet"):
                        num5, offset5 = self.check_endpoints_in_subnet(subnet.user_data.name)
                        self.populate_user_endpoint_in_subnet_tree(subnet.user_data.name, zone.user_data.name,
