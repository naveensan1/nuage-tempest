...
class ExternalIdForL3domainTest(base.BaseRouterTest):
    class MatchingVsdl3domain():
        ...
        def has_zones(self, with_external_id=None):
            ...
            self.test.assertEqual(len(self.vsd_zones), 2, "Must find exact 2 zones")
        ...
        def has_subnet(self, with_external_id=None, subnet=None, shared=False):
            ...
            for l3_subnet in self.vsd_subnets:
                if l3_subnet['name'] == subnet['id']:
                    self.test.assertEqual(shared, self.is_parent_public_zone(l3_subnet))
                    found = True
                if with_external_id is None:
                    self.test.assertIsNone(l3_subnet['externalID'])
                else:
                    self.test.assertEqual(with_external_id, l3_subnet['externalID'])
                    ...
                    self.test.assertEqual(1, len(vsd_subnets), "Subnet not found by ExternalID")
                    self.test.assertEqual(with_external_id, vsd_subnets[0]['externalID'])
        ...
        def has_permissions(self, with_external_id=None):
            for zone in self.vsd_zones:
                ...
                self.test.assertEqual(1, len(vsd_permissions), "VSD Permission not found by parent ID")

                group_external_id = ExternalId(with_external_id).at_openstack() if with_external_id else None
                if zone['name'].split('-')[1] == 'pub':
                    self.test.assertEqual(vsd_permissions[0]['permittedEntityName'],
                                          "Everybody")
                    self.has_user(group_external_id)
                else:
                    self.test.assertEqual(self.router['tenant_id'],
                ...
                if with_external_id is None:
                    ...
                    vsd_permissions = self.test.nuage_vsd_client.get_permissions(
                        ...
                        parent_id=zone['ID'],
                        filters='externalID', filter_value=with_external_id)
                    self.test.assertEqual(1, len(vsd_permissions), "VSD Permission not found by ExternalID")
        ...
        def has_group(self, with_external_id=None, for_zone=None):
            ...
            self.test.assertEqual(1, len(vsd_groups), "Group not found by VSD parent ID")

            # matching values
            self.test.assertEqual(self.router['tenant_id'], vsd_groups[0]['name'])
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(1, len(vsd_groups), "Group not found by ExternalID")
        ...
        def has_user(self, with_external_id=None):
            ...
            self.test.assertEqual(1, len(vsd_users), "User not found by VSD parent ID")

            # matching values
            self.test.assertEqual(self.router['tenant_id'], vsd_users[0]['userName'])
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(1, len(vsd_users), "User not found by ExternalID")
        ...
        def has_egress_acl_template(self, with_external_id=None):
            ...
            self.test.assertEqual(1, len(vsd_egress_acl_templates), "egress_acl_template not found by VSD parent ID")
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(1, len(vsd_egress_acl_templates), "egress_acl_template not found by ExternalID")
        ...
        def has_ingress_acl_template(self, with_external_id=None):
            ...
            self.test.assertEqual(1, len(vsd_ingress_acl_templates), "ingress_acl_template not found by VSD parent ID")
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(1, len(vsd_ingress_acl_templates), "ingress_acl_template not found by ExternalID")
        ...
        def has_forwarding_policy_template(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(1, len(vsd_forwarding_policy_templates),
    ...
    def test_router_matches_to_l3domain(self):
        ...
        if self.test_upgrade:
            vsd_l3domain = self.MatchingVsdl3domain(self, router).get_by_external_id()
        ...
        vsd_l3domain = self.MatchingVsdl3domain(self, router).get_by_external_id()
        vsd_l3domain.has_zones(with_external_id=ExternalId(router['id']).at_cms_id())
        vsd_l3domain.has_permissions(with_external_id=ExternalId(router['tenant_id']).at_cms_id())
        vsd_l3domain.has_egress_acl_template(with_external_id=ExternalId(router['id']).at_cms_id())
        vsd_l3domain.has_ingress_acl_template(with_external_id=ExternalId(router['id']).at_cms_id())
        vsd_l3domain.has_forwarding_policy_template(with_external_id=ExternalId(router['id']).at_cms_id())
    ...
    def test_subnet_attached_to_router_matches_to_l3domain(self):
        ...
        if self.test_upgrade:
            vsd_l3domain = self.MatchingVsdl3domain(self, router).get_by_external_id()
            vsd_l3domain.has_zones(with_external_id=None)
            vsd_l3domain.has_subnet(with_external_id=ExternalId(subnet['id']).at_cms_id(), subnet=subnet)
