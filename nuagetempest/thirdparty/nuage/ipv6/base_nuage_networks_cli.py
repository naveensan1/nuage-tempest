...
class BaseNuageNetworksCLITestCase(remote_cli_base_testcase.RemoteCliBaseTestCase):
    ...
    def _cli_create_os_l2_vsd_managed_dualstack_subnet(self, vsd_l2_subnet):
        ...
        subnet4 = self.create_subnet_with_args(network['name'], cidr4,
                                               "--name ", subnet_name + "-4",
        ...
        subnet6=None
        if cidr6:
            subnet6 = self.create_subnet_with_args(network['name'], cidr6,
                                                   "--name ", subnet_name + "-6",
                                                   "--ip-version 6",
                                                   "--disable-dhcp ",
                                                   "--net-partition ", net_partition,
    ...
    def _cli_create_os_l2_vsd_unmanaged_dualstack_subnet(self, vsd_l2_subnet, cidr4=None, cidr6=None):
        network_name = data_utils.rand_name('cli_network')
        network = self.create_network_with_args(network_name)
        self.addCleanup(self._delete_network, network['id'])
        self.networks.remove(network)
        ...
        net_partition = CONF.nuage.nuage_default_netpartition
        nuagenet = vsd_l2_subnet['ID']
        subnet4 = self.create_subnet_with_args(network['name'], str(cidr4),
                                               "--name ", subnet_name + "-4",
                                               "--disable-dhcp ",
                                               "--net-partition ", net_partition,
                                               "--nuagenet ", nuagenet)
        ...
        subnet6 = None
        if cidr6:
            subnet6 = self.create_subnet_with_args(network['name'], str(cidr6),
                                                   "--name ", subnet_name + "-6",
                                                   "--ip-version 6",
                                                   "--disable-dhcp ",
                                                   "--net-partition ", net_partition,
