...
class TestNuageDomainTunnelTypeCli(remote_cli_base_testcase.RemoteCliBaseTestCase):
    ...
    def skip_checks(cls):
        ...
        if not test.is_extension_enabled('nuage-router', 'network'):
    ...
    def resource_setup(cls):
        ...
        cls.me = remote_cli_base_testcase.Role.admin
    ...
    def _do_create_router_with_domain_tunnel_type(self, domain_tunnel_type):
        ...
        return self.create_router_with_args(router_name, "--tunnel-type", domain_tunnel_type)
    ...
    def test_create_router_with_default_domain_tunnel_type(self):
        get_data_center_default_domain_tunnel_type = self.get_data_center_default_domain_tunnel_type()

        created_router = self.create_router()

        # Then the router has the default tunnel type
        self.assertEqual(created_router['tunnel_type'], get_data_center_default_domain_tunnel_type)

        # When I get the router
        show_router = self.show_router(created_router['id'])

        # Then the router has the default tunnel type
        self.assertEqual(show_router['tunnel_type'], get_data_center_default_domain_tunnel_type)
    ...
    def test_create_update_router_with_tunnel_type_gre(self):
        ...
        self.update_router_with_args(created_router['id'], "--tunnel-type", domain_tunnel_type)
        ...
        get_data_center_default_domain_tunnel_type = self.get_data_center_default_domain_tunnel_type()
        self.update_router_with_args(created_router['id'], "--tunnel-type", constants.DOMAIN_TUNNEL_TYPE_DEFAULT)

        updated_router = self.show_router(created_router['id'])

        # Then I have a router in OpenStack with the data center default domain tunnel type
        self._verify_router_with_domain_tunnel_type_openstack(updated_router,
...
class TestNuageDomainTunnelTypeAsTenantCli(remote_cli_base_testcase.RemoteCliBaseTestCase):
    ...
    def test_tenant_shall_not_see_the_router_with_domain_tunnel_type(self):
        ...
        data_center_default_domain_tunnel_type = self.get_data_center_default_domain_tunnel_type()
        router_name = data_utils.rand_name('test-router')
        ...
        self.assertEqual(router_as_admin['tunnel_type'], data_center_default_domain_tunnel_type)
    ...
    def test_tenant_shall_not_create_router_with_domain_tunnel_type(self):
        ...
        self._as_admin()
        created_router = self.create_router_with_args(router_name, "--tunnel-type", domain_tunnel_type)
        self.assertEqual(created_router['tunnel_type'], domain_tunnel_type)

        # admin can update a router with domain tunnel type
        self._as_admin()
        new_domain_tunnel_type = constants.DOMAIN_TUNNEL_TYPE_GRE
        self.update_router_with_args(created_router['id'], "--tunnel-type", new_domain_tunnel_type)
        updated_router = self.show_router(created_router['id'])
        self.assertEqual(updated_router['tunnel_type'], new_domain_tunnel_type)
