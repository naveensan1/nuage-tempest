...
class ExternalIdForL3domainTest(base.BaseRouterTest):
    class MatchingVsdl3domain():
        ...
        def has_zones(self, with_external_id=None):
            ...
            for zone in self.vsd_zones:
                if with_external_id is None:
                    ...
                    self.test.assertEqual(zone['externalID'],
                                          self.test.nuage_vsd_client.get_vsd_external_id(with_external_id))
        ...
        def has_subnet(self, with_external_id=None, subnet=None, shared=False):
            ...
            for l3_subnet in vsd_subnets:
                ...
                if with_external_id is None:
                    ...
                    self.test.assertEqual(l3_subnet['externalID'],
                                          self.test.nuage_vsd_client.get_vsd_external_id(with_external_id))
        ...
        def has_permissions(self, with_external_id=None):
            for zone in self.vsd_zones:
                ...
                if zone['name'].split('-')[1] == 'pub':
                    ...
                    self.has_group(with_external_id, for_zone=zone)
                    self.has_user(with_external_id)
        ...
        def has_egress_acl_template(self, with_external_id=None):
            ...
            if with_external_id is None:
                ...
                self.test.assertEqual(self.test.nuage_vsd_client.get_vsd_external_id(self.router['id']),
    ...
    def test_router_matches_to_l3domain(self):
        ...
        if self.test_upgrade:
            ...
            vsd_l3domain.has_ingress_advanced_forward_acl_template(with_external_id=None)

            upgrade_script.do_run_upgrade_script()

        vsd_l3domain = self.MatchingVsdl3domain(self, router).get_by_external_id()
        vsd_l3domain.has_zones(with_external_id=router['id'])
        vsd_l3domain.has_permissions(with_external_id=router['tenant_id'])
        vsd_l3domain.has_egress_acl_template(with_external_id=router['id'])
        vsd_l3domain.has_ingress_acl_template(with_external_id=router['id'])
        vsd_l3domain.has_ingress_advanced_forward_acl_template(with_external_id=router['id'])
    ...
    def test_subnet_attached_to_router_matches_to_l3domain(self):
        ...
        if self.test_upgrade:
            vsd_l3domain = self.MatchingVsdl3domain(self, router).get_by_external_id()
            vsd_l3domain.has_zones(with_external_id=None)
            vsd_l3domain.has_subnet(with_external_id=subnet['id'], subnet=subnet)
        ...
        # vsd_l3domain.has_permissions(with_external_id=subnet['tenant_id'])
        # vsd_l3domain.has_egress_acl_template(with_external_id=subnet['id'])
        # vsd_l3domain.has_ingress_acl_template(with_external_id=subnet['id'])
        # vsd_l3domain.has_ingress_advanced_forward_acl_template(with_external_id=router['id'])
