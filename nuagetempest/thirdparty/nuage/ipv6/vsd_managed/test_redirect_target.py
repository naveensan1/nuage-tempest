...
class VSDManagedRedirectTargetTest(VsdTestCaseMixin,
    ...
    def test_create_delete_os_redirection_target_l3_mgd_subnet(self):
        ...
        my_rt_found = self._find_redirect_target_in_list(os_redirect_target['nuage_redirect_target']['id'], subnet4)
        ...
        #  When I create a redirection-target in the VSD-L3-Managed-Subnet
        os_redirect_target6 = self._create_redirect_target_in_l3_subnet(subnet6)
        # Then I expect the redirection-target in my list
        my_rt_found = self._find_redirect_target_in_list(os_redirect_target['nuage_redirect_target']['id'], subnet6)
        self.assertTrue(my_rt_found, "Did not find my redirect-target in the list")
        # and check on VSD
        vsd_redirect_target = self.nuage_vsd_client.get_redirection_target(
            constants.DOMAIN, vsd_l3_domain['ID'], filters='ID',
        ...
        port_present = self._check_port_in_show_redirect_target(rtport, os_redirect_target6)
        message = "Associated port not present in show nuage redirect target response"
        self.assertTrue(port_present, message)
        ...
        self.nuage_network_client.delete_redirection_target(os_redirect_target6['nuage_redirect_target']['id'])
        # I expect the redirect-target to be gone from the list
        my_rt_found = self._find_redirect_target_in_list(os_redirect_target['nuage_redirect_target']['id'], subnet6)
        self.assertEqual(False, my_rt_found,
                         message="Deleted nuage_redirect_target still present in subnet")
    ...
    #     fip1 = self.nuage_vsd_client.claim_floatingip(vsd_l3_domain['ID'], vsd_fip_pool['ID'])[0]
    #     fip2 = self.nuage_vsd_client.claim_floatingip(vsd_l3_domain['ID'], vsd_fip_pool['ID'])[0]
    #
    #     # When I retrieve the nuage-floatingIP-list of the OpenStack IPv4 subnet
    #     fip_list = self.nuage_network_client.list_nuage_floatingip_by_subnet(subnet4['id'])
    ...
    #
    #     # When I retrieve the nuage-floatingIP-list of the OpenStack IPv6 subnet
    #     fip_list = self.nuage_network_client.list_nuage_floatingip_by_subnet(subnet6['id'])
    #     # Then I expect the VSD-floatingIP in my list
    ...
    #     self.assertThat(port['nuage_floatingip'], ContainsDict({'id': Equals(fip1['ID'])}))
    #
    ...
    #     if CONF.nuage_sut.nuage_plugin_mode != 'ml2':
    #         fip_present = self._check_fip_in_port_show(port['id'], fip1[0]['ID'])
