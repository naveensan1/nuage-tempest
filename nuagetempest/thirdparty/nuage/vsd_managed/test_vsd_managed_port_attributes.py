...
class VSDManagedRedirectTargetTest(base_vsd_managed_port_attributes.BaseVSDManagedPortAttributes):
    ...
    def test_create_delete_several_redirection_targets_l2_mgd_subnet(self):
        ...
        # Then I expect the redirection-target in my list
        for i in range(SEVERAL_REDIRECT_TARGETS):
            my_os_rt_found = self._find_redirect_target_in_list(os_redirect_targets[i]['nuage_redirect_target']['id'],
                                                                subnet)
            self.assertTrue(my_os_rt_found, "Did not find my redirect-target in the list")
            my_vsd_rt_found = self._find_redirect_target_in_list(vsd_redirect_targets[i]['nuage_redirect_target']['id'],
                                                                 subnet)
    ...
    def test_create_os_redirection_target_same_name_diff_l2_mgd_subnet(self):
        ...
        my_rt_found_x = self._find_redirect_target_in_list(os_redirect_target_x['nuage_redirect_target']['id'],
                                                           subnet_x)
    ...
    def test_associate_two_port_same_l2_os_redirection_target_neg(self):
        ...
        if NUAGE_FEATURES.ml2_limited_exceptions:
            ...
            if Release(CONF.nuage_sut.release) < Release('5.0'):
                expected_exception = exceptions.ServerFault
    ...
    def test_create_os_l2_redirection_target_redundancy_enabled_neg(self):
        ...
        # When I try to creeate  a redirection target
        # I expect an error
        msg = "Cannot have more than 1 vPort under a redirectiontarget with redundancy disabled"
        expected_exception = exceptions.BadRequest
    ...
    def test_os_redirection_targets_bad_insertion_mode_neg(self):
        ...
        if CONF.nuage_sut.nuage_plugin_mode == 'ml2':
            ...
            # TODO: VSD-14421 Need to change expected exception and error message
            LOG.warning("VSD-14421: bad insertion mode: ServerFault iso BadRequest")
            expected_exception = exceptions.ServerFault
            msg = "Nuage API"
