...
class NuageFipRateLimitBase(base.BaseNetworkTest):
    ...
    def _verify_fip_openstack(self, port, created_floating_ip, default_rate_limit):
        ...
        self.assertEqual(str(created_floating_ip['nuage_fip_rate']), str(default_rate_limit))
    ...
    def _create_fip_with_fip_rate_limit(self, port, fip_rate_limit):
        ...
        created_floating_ip = self._do_create_fip_for_port_with_rate_limit(port['id'], fip_rate_limit)
        ...
        self._verify_fip_vsd(port, created_floating_ip, openstack_to_vsd(fip_rate_limit))

        return created_floating_ip

    def _update_fip_with_fip_rate_limit(self, port, floating_ip, fip_rate_limit):
        # When I create a fip with default rate limit
        updated_floating_ip = self._do_update_fip_to_rate_limit(floating_ip['id'], fip_rate_limit)

        # Then I got a valid OpenStack FIP with the default rate limit
        self._verify_fip_openstack(port, updated_floating_ip, fip_rate_limit)

        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, updated_floating_ip, openstack_to_vsd(fip_rate_limit))
    ...
    def _show_fip_with_fip_rate_limit(self, port, floating_ip, fip_rate_limit):
        ...
        # Then I got a valid VSD FIP with the default rate limit
        self._verify_fip_vsd(port, floating_ip, openstack_to_vsd(fip_rate_limit))
